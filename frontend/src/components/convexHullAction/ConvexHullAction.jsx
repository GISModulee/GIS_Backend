...
export default function ConvexHullAction({ onCancel }) {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      console.log("[ConvexHullAction] parsed geom:", geom);
      ...
      const saveRes = await layerService.createFeature({
        ...
        properties: { name, color: "#d97706" },
      });

      // Reload layers from backend to render the new layer and feature
      await loadLayersFromBackend();

      toast.success(saveRes?.message || response?.message || newLayer?.message || "✓ Convex Hull layer created and added to map!", { id: toastId });
    ...
    <div className="space-y-2 pt-1 border-t border-gray-100 dark:border-gray-800">
      ...
      <div>
        ...
        <input
          ...
          placeholder="Convex Hull result"
          disabled={loading}
      ...
      >
        {loading ? "Saving..." : "Run Convex Hull"}
      </button>
    </div>
  );
}
