...
export default function FeatureSelector({ label, value, onChange, items, placeholder = "— select —", disabled = false }) {
  const [isOpen, setIsOpen] = useState(false);
  const dispatch = useDispatch();

  // Find currently selected feature
  const selectedFeature = items
    .flatMap((l) => l.features || [])
    .find((f) => f.localId === value);
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="truncate">{selectedFeature ? selectedFeature.name : placeholder}</span>
              ...
              <div key={lyr.localId}>
                ...
                {lyr.features?.map((f) => (
                  <div
                    ...
                    className="px-3 py-1 hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:text-blue-600 dark:hover:text-blue-400 cursor-pointer transition-colors"
                  >
                    {f.name}
                  </div>
                ))}
