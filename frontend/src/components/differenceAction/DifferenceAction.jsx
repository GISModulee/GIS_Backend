...
export default function DifferenceAction({ onCancel }) {
  ...
  const handleRun = async () => {
    if (!Array.isArray(selectedFeatures) || selectedFeatures.length < 2) {
      toast.error("Please select at least 2 features to perform Difference.");
      return;
    }
    ...
    setLoading(true);
    const toastId = toast.loading("Computing and saving difference layer...");
    try {
      const defaultName = `Difference (${selectedObjs.map(f => f.name || "Feature").join(" - ")})`;
      const name = resultName.trim() || defaultName;

      const response = await layerService.runDifference({
      ...
      const saveRes = await layerService.createFeature({
        ...
        properties: { name, color: "#ef4444" },
      });

      // Reload layers from backend to render the new layer and feature
      await loadLayersFromBackend();

      toast.success(saveRes?.message || response?.message || newLayer?.message || "✓ Difference layer created and added to map!", { id: toastId });
