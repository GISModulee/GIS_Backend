...
import {
  dispatchFeatureCreated,
  dispatchFeatureBatchCreated,
  dispatchFeatureUpdated,
} from "@/utils/featureNormalization.js";

export function useFeatureWebSocket({ caseId }) {
  ...
  const flushBuffer = () => {
    Object.keys(batchBufferRef.current).forEach((layerLocalId) => {
      ...
      if (features.length > 0) {
        ...
        dispatchFeatureBatchCreated(dispatch, layerLocalId, features);
      }
    });
    ...
    pendingFeaturesRef.current.forEach((item) => {
      ...
      if (layerExists) {
        if (item.type === "batch") {
          ...
          dispatchFeatureBatchCreated(dispatch, item.layerLocalId, item.features);
        } else {
          ...
          if (!alreadyExists) {
            ...
            dispatchFeatureCreated(dispatch, item.layerLocalId, f);
          }
        }
      } else {
        remaining.push(item);
      ...
      socket.onmessage = async (event) => {
        ...
        try {
          ...
          if (message.event === "feature.batch_created" && Array.isArray(message.features)) {
            const currentLayerExists = itemsRef.current.some((l) => l.localId === layerLocalId);
            if (currentLayerExists) {
              dispatchFeatureBatchCreated(dispatch, layerLocalId, message.features);
            } else {
          ...
          if (message.event === "feature.updated" && message.feature) {
            ...
            if (exists) {
              dispatchFeatureUpdated(dispatch, layerLocalId, featureLocalId, f);
            }
          }
